#!/usr/bin/env python3
"""
Import GA4 Customers

This script imports customer data from the GA4 Customers.csv file into the garage system database.
It handles the conversion from string IDs to integer IDs and maps the CSV columns to the database columns.
"""
        ...
        with open(csv_path, 'r', encoding='utf-8', errors='replace') as f:
            ...
            for row in reader:
                try:
                    ...
                    ga4_id = clean_value(row[header_map['_id']])
                    ...
                    # Skip if no name
                    if not name_parts:
                        logger.warning(f"Skipping customer {ga4_id} with no name")
                        continue
...
def main():
    """Main function"""
    logger.info("Starting Import GA4 Customers")
